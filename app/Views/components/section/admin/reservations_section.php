...
<div class="space-y-8">
    ...
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-3xl md:text-4xl font-serif font-bold text-primary-dark">Réservations</h1>
            <p class="text-gray-600 mt-1">Gérez les demandes de réservation des clients</p>
        </div>
        ...
        <div class="overflow-x-auto">
            <?php if (empty($reservations)): ?>
                <div class="p-12 text-center">
                    ...
                    <p class="text-gray-500 font-medium">Aucune réservation pour le moment</p>
                </div>
            <?php else: ?>
                <table class="w-full">
                    ...
                    <tbody class="divide-y divide-gray-200">
                        ...
                        ?>
                            <tr class="reservation-row hover:bg-gray-50 transition-colors" data-status="<?= $status ?>">
                                <td class="px-6 py-4">
                                    <div class="font-medium text-gray-900"><?= esc($reservation['customer_name']) ?></div>
                                    <div class="text-sm text-gray-500"><?= esc($reservation['customer_email']) ?></div>
                                    <?php if (!empty($reservation['customer_phone'])): ?>
                                        <div class="text-xs text-gray-400 flex items-center gap-1 mt-0.5">
                                            <i data-lucide="phone" class="w-3 h-3"></i>
                                            <?= esc($reservation['customer_phone']) ?>
                                        </div>
                                    <?php endif; ?>
                                </td>
                                <td class="px-6 py-4">
                                    <div class="font-medium text-gray-900"><?= esc($reservation['product_title'] ?? 'N/A') ?></div>
                                    <?php if (!empty($reservation['category_name'])): ?>
                                        <div class="text-xs text-gray-500"><?= esc($reservation['category_name']) ?></div>
                                    <?php endif; ?>
                                    <?php if (!empty($reservation['price'])): ?>
                                        <div class="text-sm text-gray-600 mt-1"><?= number_format($reservation['price'], 2, ',', ' ') ?> €</div>
                                    <?php endif; ?>
                                </td>
                                ...
                                <td class="px-6 py-4">
                                    <div class="text-sm text-gray-900"><?= date('d/m/Y', strtotime($reservation['created_at'])) ?></div>
                                    <div class="text-xs text-gray-500"><?= date('H:i', strtotime($reservation['created_at'])) ?></div>
                                </td>
                                <td class="px-6 py-4 text-center">
                                    <div class="flex items-center justify-center gap-2">
                                        <button onclick="viewDetails(<?= $reservation['id'] ?>)" 
                                                class="p-2 rounded-lg hover:bg-blue-50 text-blue-600 transition">
                                            <i data-lucide="eye" class="w-4 h-4"></i>
                                        </button>
                                        <button onclick="updateStatus(<?= $reservation['id'] ?>)" 
...
// Par défaut, afficher les nouvelles réservations
