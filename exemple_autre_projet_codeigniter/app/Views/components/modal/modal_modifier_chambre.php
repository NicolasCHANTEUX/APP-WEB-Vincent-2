 ...
 * 
 * Formulaire modal pour modifier une chambre existante
 */

$modalId = 'modal-modifier-chambre';

ob_start();
?>
<form id="form-modifier-chambre" method="POST" action="" class="space-y-6">
    <input type="hidden" name="id_chambre" value="">

    <div>
        <h4 class="text-lg font-semibold text-primary mb-4">Modifier la Chambre</h4>

        <div class="mb-4">
            <?php
            helper('chambre');
            // Préparer les options depuis la base de données
            $options = [];
...
$footer = '
    <div class="flex gap-3 justify-end">
        ...
        <button 
            type="submit"
            form="form-modifier-chambre"
            class="px-6 py-2 bg-primary text-secondary-foreground rounded-lg hover:bg-primary/90 transition-all font-semibold"
        >
            Enregistrer
        </button>
    </div>
